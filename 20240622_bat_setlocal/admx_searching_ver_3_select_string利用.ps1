...
# 修正中
# -----------------------------------------------------
function txt_searching_output_Select-String {
    Param(
        $Arg1,  # path
        $Arg2,  # input_filename
        $Arg3   # output_filename
	...
	$result=Select-String -Path ($Arg1 + $Arg2) -Pattern "<policy name=" -Context 0, 1 | % { $_.Context.PreContext + $_.Line + $_.Context.PostContext }
	Write-Host($result)
	
	Select-String -Path ($fileName_input) -Pattern "<policy name=" -Context 0, 1 | % { $_.Context.PreContext + $_.Line + $_.Context.PostContext } | Set-Content $fileName_output
...
#region startup
    ...
    # Write-Host($string)
    txt_searching_output_Select-String $path "\SystemRestore.admx.txt" "\SystemRestore.admx_ouput.txt"
#endregion
# -----------------------------